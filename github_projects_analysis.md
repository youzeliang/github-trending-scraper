# GitHub Trending 项目分析
> 生成时间: 2026-04-11 18:37:44
> 项目总数: 234

---
## 1. agrinman/tunnelto
- **GitHub**: https://github.com/agrinman/tunnelto
- **语言**: Rust
- **描述**: 一个将本地服务器暴露到互联网的工具，支持内网穿透
- **主要特性**: 无需公网IP即可让外部访问本地服务

---
## 2. jaywcjlove/awesome-mac
- **GitHub**: https://github.com/jaywcjlove/awesome-mac
- **语言**: JavaScript
- **描述**: 精选的Mac应用和工具列表
- **主要特性**: 收录优质macOS软件、效率工具、开发环境配置

---
## 3. cocoindex-io/cocoindex
- **GitHub**: https://github.com/cocoindex-io/cocoindex
- **语言**: Go
- **描述**: 代码索引和语义搜索工具
- **主要特性**: 支持代码库语义搜索和代码理解

---
## 4. sgl-project/mini-sglang
- **GitHub**: https://github.com/sgl-project/mini-sglang
- **语言**: Python
- **描述**: 轻量级SGLang运行时
- **主要特性**: 高效的大语言模型推理框架，支持流式输出

---
## 5. tursodatabase/turso
- **GitHub**: https://github.com/tursodatabase/turso
- **语言**: Go
- **描述**: 边缘数据库，基于libSQL（SQLite分支）
- **主要特性**: 全球分布式SQLite数据库，支持边缘计算场景

---
## 6. datawhalechina/hello-agents
- **GitHub**: https://github.com/datawhalechina/hello-agents
- **语言**: Python
- **描述**: AI Agent入门教程
- **主要特性**: Datawhale出品的AI Agent学习资源

---
## 7. trustedsec/social-engineer-toolkit
- **GitHub**: https://github.com/trustedsec/social-engineer-toolkit
- **语言**: Python
- **描述**: 社会工程学工具集
- **主要特性**: 安全研究和渗透测试工具

---
## 8. trimstray/the-book-of-secret-knowledge
- **GitHub**: https://github.com/trimstray/the-book-of-secret-knowledge
- **语言**: Shell
- **描述**: 黑客和安全工具知识库
- **主要特性**: 系统管理员和安全工程师的参考手册

---
## 9. ZJU-LLMs/Foundations-of-LLMs
- **GitHub**: https://github.com/ZJU-LLMs/Foundations-of-LLMs
- **语言**: Python
- **描述**: 浙江大学LLM基础教程
- **主要特性**: 大语言模型理论和实践课程

---
## 10. anthropics/skills
- **GitHub**: https://github.com/anthropics/skills
- **语言**: Python
- **描述**: Anthropic Claude技能库
- **主要特性**: 官方Claude扩展和技能集

---
## 11. safety-research/bloom
- **GitHub**: https://github.com/safety-research/bloom
- **语言**: Python
- **描述**: 安全研究工具
- **主要特性**: Bloom安全研究相关项目

---
## 12. xerrors/Yuxi-Know
- **GitHub**: https://github.com/xerrors/Yuxi-Know
- **语言**: Python
- **描述**: 个人知识管理系统
- **主要特性**: 笔记和知识整理工具

---
## 13. apurvsinghgautam/robin
- **GitHub**: https://github.com/apurvsinghgautam/robin
- **语言**: Python
- **描述**: AI助手项目
- **主要特性**: 个人AI助手实现

---
## 14. ostris/ai-toolkit
- **GitHub**: https://github.com/ostris/ai-toolkit
- **语言**: Python
- **描述**: AI艺术创作工具包
- **主要特性**: 图像生成和编辑的AI工具集合

---
## 15. Shubhamsaboo/awesome-llm-apps
- **GitHub**: https://github.com/Shubhamsaboo/awesome-llm-apps
- **语言**: Python
- **描述**: LLM应用精选列表
- **主要特性**: 收集优秀的LLM应用案例

---
## 16. NanmiCoder/MediaCrawler
- **GitHub**: https://github.com/NanmiCoder/MediaCrawler
- **语言**: Python
- **描述**: 社交媒体数据爬虫
- **主要特性**: 支持小红书、微博等平台数据采集

---
## 17. google-gemini/computer-use-preview
- **GitHub**: https://github.com/google-gemini/computer-use-preview
- **语言**: Python
- **描述**: Google Gemini计算机控制预览
- **主要特性**: AI代理控制计算机的实验性功能

---
## 18. nocodb/nocodb
- **GitHub**: https://github.com/nocodb/nocodb
- **语言**: TypeScript
- **描述**: 开源Airtable替代品
- **主要特性**: 可视化数据库管理界面，支持SQLite/MySQL/PostgreSQL

---
## 19. GitHubDaily/GitHubDaily
- **GitHub**: https://github.com/GitHubDaily/GitHubDaily
- **语言**: Python
- **描述**: GitHub日报
- **主要特性**: 每日分享GitHub优质项目

---
## 20. livekit/agents
- **GitHub**: https://github.com/livekit/agents
- **语言**: Python
- **描述**: LiveKit AI代理框架
- **主要特性**: 实时音视频AI代理开发框架

---
## 21. shareAI-lab/learn-claude-code
- **GitHub**: https://github.com/shareAI-lab/learn-claude-code
- **语言**: Python
- **描述**: Claude Code学习指南
- **主要特性**: Claude Code使用教程和最佳实践

---
## 22. Lissy93/web-check
- **GitHub**: https://github.com/Lissy93/web-check
- **语言**: JavaScript
- **描述**: 网站健康检查工具
- **主要特性**: 分析网站性能、安全性、SEO等指标

---
## 23. anthropics/claude-code-action
- **GitHub**: https://github.com/anthropics/claude-code-action
- **语言**: TypeScript
- **描述**: Claude Code GitHub Action
- **主要特性**: 在CI/CD中集成Claude Code

---
## 24. marcelscruz/public-apis
- **GitHub**: https://github.com/marcelscruz/public-apis
- **语言**: Python
- **描述**: 免费公共API列表
- **主要特性**: 汇总可免费使用的API接口

---
## 25. kirodotdev/Kiro
- **GitHub**: https://github.com/kirodotdev/Kiro
- **语言**: TypeScript
- **描述**: 开发者工具
- **主要特性**: Kiro开发者工具集

---
## 26. VectifyAI/PageIndex
- **GitHub**: https://github.com/VectifyAI/PageIndex
- **语言**: Python
- **描述**: 页面索引工具
- **主要特性**: 网页内容索引和搜索

---
## 27. ChromeDevTools/chrome-devtools-mcp
- **GitHub**: https://github.com/ChromeDevTools/chrome-devtools-mcp
- **语言**: TypeScript
- **描述**: Chrome DevTools MCP服务器
- **主要特性**: Chrome开发者工具的MCP协议实现

---
## 28. patchy631/ai-engineering-hub
- **GitHub**: https://github.com/patchy631/ai-engineering-hub
- **语言**: Python
- **描述**: AI工程中心
- **主要特性**: AI工程实践资源

---
## 29. MiroMindAI/MiroThinker
- **GitHub**: https://github.com/MiroMindAI/MiroThinker
- **语言**: Python
- **描述**: AI思考框架
- **主要特性**: 多模态AI思考和推理

---
## 30. xpipe-io/xpipe
- **GitHub**: https://github.com/xpipe-io/xpipe
- **语言**: Go
- **描述**: 命令行管道工具
- **主要特性**: Linux/Unix管道命令的现代化替代

---
## 31. NevaMind-AI/memU
- **GitHub**: https://github.com/NevaMind-AI/memU
- **语言**: Python
- **描述**: 记忆管理AI
- **主要特性**: AI驱动的个人记忆系统

---
## 32. HKUDS/VideoRAG
- **GitHub**: https://github.com/HKUDS/VideoRAG
- **语言**: Python
- **描述**: 视频检索增强生成
- **主要特性**: 视频内容的RAG检索系统

---
## 33. numman-ali/opencode-openai-codex-auth
- **GitHub**: https://github.com/numman-ali/opencode-openai-codex-auth
- **语言**: Python
- **描述**: OpenAI Codex认证
- **主要特性**: OpenCode认证相关工具

---
## 34. NVlabs/alpasim
- **GitHub**: https://github.com/NVlabs/alpasim
- **语言**: Python
- **描述**: NVIDIA模拟器
- **主要特性**: NVIDIA相关模拟工具

---
## 35. tailwindlabs/tailwindcss
- **GitHub**: https://github.com/tailwindlabs/tailwindcss
- **语言**: JavaScript
- **描述**: 实用优先的CSS框架
- **主要特性**: 原子化CSS框架，快速构建现代界面

---
## 36. netbirdio/netbird
- **GitHub**: https://github.com/netbirdio/netbird
- **语言**: Go
- **描述**: 开源VPN替代方案
- **主要特性**: 基于WireGuard的点对点安全网络

---
## 37. C4illin/ConvertX
- **GitHub**: https://github.com/C4illin/ConvertX
- **语言**: JavaScript
- **描述**: 多功能转换工具
- **主要特性**: 支持文件、音频、视频等多种格式转换

---
## 38. bytedance/UI-TARS-desktop
- **GitHub**: https://github.com/bytedance/UI-TARS-desktop
- **语言**: Python
- **描述**: 字节跳动UI代理桌面应用
- **主要特性**: AI驱动的UI自动化工具

---
## 39. Johnshall/Shadowrocket-ADBlock-Rules-Forever
- **GitHub**: https://github.com/Johnshall/Shadowrocket-ADBlock-Rules-Forever
- **语言**: JavaScript
- **描述**: Shadowrocket广告拦截规则
- **主要特性**: 持续更新的iOS代理规则

---
## 40. frankbria/ralph-claude-code
- **GitHub**: https://github.com/frankbria/ralph-claude-code
- **语言**: Python
- **描述**: Ralph AI助手
- **主要特性**: Ralph品牌的Claude集成

---
## 41. obra/superpowers
- **GitHub**: https://github.com/obra/superpowers
- **语言**: Rust
- **描述**: 游戏开发工具
- **主要特性**: 独立游戏开发辅助工具

---
## 42. opf/openproject
- **GitHub**: https://github.com/opf/openproject
- **语言**: Ruby
- **描述**: 开源项目管理软件
- **主要特性**: 功能完整的项目管理和团队协作平台

---
## 43. hacksider/Deep-Live-Cam
- **GitHub**: https://github.com/hacksider/Deep-Live-Cam
- **语言**: Python
- **描述**: 实时AI换脸工具
- **主要特性**: 使用深度学习实现实时人脸替换

---
## 44. twentyhq/twenty
- **GitHub**: https://github.com/twentyhq/twenty
- **语言**: TypeScript
- **描述**: 开源CRM系统
- **主要特性**: 现代开源客户关系管理平台

---
## 45. twitter/twemoji
- **GitHub**: https://github.com/twitter/twemoji
- **语言**: JavaScript
- **描述**: Twitter开源表情符号
- **主要特性**: Twitter开源的表情图标库

---
## 46. home-assistant/home-assistant.io
- **GitHub**: https://github.com/home-assistant/home-assistant.io
- **语言**: Python
- **描述**: 智能家居平台文档
- **主要特性**: Home Assistant配置和使用文档

---
## 47. gyoridavid/ai_agents_az
- **GitHub**: https://github.com/gyoridavid/ai_agents_az
- **语言**: Python
- **描述**: Azure AI代理
- **主要特性**: 微软Azure上的AI代理实现

---
## 48. steveyegge/beads
- **GitHub**: https://github.com/steveyegge/beads
- **语言**: JavaScript
- **描述**: 编程语言实现
- **主要特性**: 用JavaScript实现的教育性编程语言

---
## 49. DioxusLabs/dioxus
- **GitHub**: https://github.com/DioxusLabs/dioxus
- **语言**: Rust
- **描述**: 声明式UI框架
- **主要特性**: 高性能跨平台UI框架，支持Web/桌面/移动端

---
## 50. ruvnet/claude-flow
- **GitHub**: https://github.com/ruvnet/claude-flow
- **语言**: Python
- **描述**: Claude工作流自动化
- **主要特性**: Claude AI工作流编排工具

---
## 51. mpv-player/mpv
- **GitHub**: https://github.com/mpv-player/mpv
- **语言**: C
- **描述**: 多功能跨平台播放器
- **主要特性**: 支持几乎所有音视频格式的开源播放器

---
## 52. OpenBMB/ChatDev
- **GitHub**: https://github.com/OpenBMB/ChatDev
- **语言**: Python
- **描述**: 多代理软件开发框架
- **主要特性**: 使用LLM驱动的虚拟软件公司

---
## 53. icloud-photos-downloader/icloud_photos_downloader
- **GitHub**: https://github.com/icloud-photos-downloader/icloud_photos_downloader
- **语言**: Python
- **描述**: iCloud照片下载器
- **主要特性**: 备份iCloud照片到本地

---
## 54. blakeblackshear/frigate
- **GitHub**: https://github.com/blakeblackshear/frigate
- **语言**: Python
- **描述**: 开源AI家庭监控
- **主要特性**: NVR解决方案，支持本地AI检测

---
## 55. chidiwilliams/buzz
- **GitHub**: https://github.com/chidiwilliams/buzz
- **语言**: TypeScript
- **描述**: 离线语音转文字工具
- **主要特性**: 支持 Whisper 的桌面应用

---
## 56. adam-maj/tiny-gpu
- **GitHub**: https://github.com/adam-maj/tiny-gpu
- **语言**: Verilog
- **描述**: GPU架构学习项目
- **主要特性**: 用Verilog实现的教学级GPU

---
## 57. Free-TV/IPTV
- **GitHub**: https://github.com/Free-TV/IPTV
- **语言**: Python
- **描述**: IPTV频道列表
- **主要特性**: 收集免费IPTV直播源

---
## 58. onlook-dev/onlook
- **GitHub**: https://github.com/onlook-dev/onlook
- **语言**: Rust
- **描述**: 设计到代码工具
- **主要特性**: 设计稿自动转换为代码

---
## 59. badlogic/pi-mono
- **GitHub**: https://github.com/badlogic/pi-mono
- **语言**: C
- **描述**: 游戏引擎
- **主要特性**: 轻量级跨平台游戏引擎

---
## 60. KeygraphHQ/shannon
- **GitHub**: https://github.com/KeygraphHQ/shannon
- **语言**: Python
- **描述**: 图神经网络工具
- **主要特性**: 知识图谱和图神经网络库

---
## 61. dev-sec/ansible-collection-hardening
- **GitHub**: https://github.com/dev-sec/ansible-collection-hardening
- **语言**: Python
- **描述**: Ansible安全加固集合
- **主要特性**: 服务器安全配置自动化

---
## 62. grab/cursor-talk-to-figma-mcp
- **GitHub**: https://github.com/grab/cursor-talk-to-figma-mcp
- **语言**: TypeScript
- **描述**: Figma MCP服务器
- **主要特性**: Figma设计工具的MCP集成

---
## 63. zoicware/RemoveWindowsAI
- **GitHub**: https://github.com/zoicware/RemoveWindowsAI
- **语言**: Python
- **描述**: Windows AI工具
- **主要特性**: Windows系统AI增强工具

---
## 64. rancher/rancher
- **GitHub**: https://github.com/rancher/rancher
- **语言**: Go
- **描述**: 容器管理平台
- **主要特性**: Kubernetes集群管理界面

---
## 65. eigent-ai/eigent
- **GitHub**: https://github.com/eigent-ai/eigent
- **语言**: Python
- **描述**: AI安全研究
- **主要特性**: AI安全和对齐研究

---
## 66. cilium/cilium
- **GitHub**: https://github.com/cilium/cilium
- **语言**: Go
- **描述**: 云原生网络
- **主要特性**: 基于eBPF的Kubernetes网络和安全

---
## 67. wavetermdev/waveterm
- **GitHub**: https://github.com/wavetermdev/waveterm
- **语言**: TypeScript
- **描述**: 现代化终端
- **主要特性**: 支持图形和命令的下一代终端

---
## 68. ultralytics/ultralytics
- **GitHub**: https://github.com/ultralytics/ultralytics
- **语言**: Python
- **描述**: YOLO目标检测框架
- **主要特性**: 计算机视觉模型训练和推理

---
## 69. google-ai-edge/mediapipe
- **GitHub**: https://github.com/google-ai-edge/mediapipe
- **语言**: JavaScript/C++
- **描述**: Google AI感知框架
- **主要特性**: 人脸检测、手势识别等AI模型

---
## 70. facebookresearch/sam-audio
- **GitHub**: https://github.com/facebookresearch/sam-audio
- **语言**: Python
- **描述**: Meta音频分割模型
- **主要特性**: 音频源分离模型

---
## 71. ComposioHQ/awesome-claude-skills
- **GitHub**: https://github.com/ComposioHQ/awesome-claude-skills
- **语言**: Python
- **描述**: Claude技能精选
- **主要特性**: Composio出品的Claude扩展集合

---
## 72. nautechsystems/nautilus_trader
- **GitHub**: https://github.com/nautechsystems/nautilus_trader
- **语言**: Python
- **描述**: 加密货币交易框架
- **主要特性**: 高性能量化交易系统

---
## 73. DataTalksClub/data-engineering-zoomcamp
- **GitHub**: https://github.com/DataTalksClub/data-engineering-zoomcamp
- **语言**: Python
- **描述**: 数据工程课程
- **主要特性**: 免费数据工程学习课程

---
## 74. alirezarezvani/claude-skills
- **GitHub**: https://github.com/alirezarezvani/claude-skills
- **语言**: Python
- **描述**: Claude技能集合
- **主要特性**: 社区贡献的Claude技能

---
## 75. lukasz-madon/awesome-remote-job
- **GitHub**: https://github.com/lukasz-madon/awesome-remote-job
- **语言**: Python
- **描述**: 远程工作资源列表
- **主要特性**: 远程Job和远程工作资源

---
## 76. EveryInc/compound-engineering-plugin
- **GitHub**: https://github.com/EveryInc/compound-engineering-plugin
- **语言**: TypeScript
- **描述**: 工程插件
- **主要特性**: 复合工程工具集

---
## 77. liyupi/ai-guide
- **GitHub**: https://github.com/liyupi/ai-guide
- **语言**: Python
- **描述**: AI开发者指南
- **主要特性**: 程序员AI转型指南

---
## 78. block/goose
- **GitHub**: https://github.com/block/goose
- **语言**: Python
- **描述**: AI代理开发框架
- **主要特性**: Block开源的AI代理工具

---
## 79. Asabeneh/30-Days-Of-Python
- **GitHub**: https://github.com/Asabeneh/30-Days-Of-Python
- **语言**: Python
- **描述**: 30天Python学习
- **主要特性**: 系统化Python学习课程

---
## 80. OpenBMB/UltraRAG
- **GitHub**: https://github.com/OpenBMB/UltraRAG
- **语言**: Python
- **描述**: 高级RAG框架
- **主要特性**: 检索增强生成的优化实现

---
## 81. lyogavin/airllm
- **GitHub**: https://github.com/lyogavin/airllm
- **语言**: Python
- **描述**: LLM推理优化
- **主要特性**: 大模型推理加速库

---
## 82. supermemoryai/supermemory
- **GitHub**: https://github.com/supermemoryai/supermemory
- **语言**: TypeScript
- **描述**: AI记忆库
- **主要特性**: AI驱动的个人知识管理

---
## 83. asgeirtj/system_prompts_leaks
- **GitHub**: https://github.com/asgeirtj/system_prompts_leaks
- **语言**: Python
- **描述**: 系统提示词泄露研究
- **主要特性**: 收集和分析系统提示词

---
## 84. kubernetes/ingress-nginx
- **GitHub**: https://github.com/kubernetes/ingress-nginx
- **语言**: Go
- **描述**: Kubernetes Ingress控制器
- **主要特性**: 官方NGINX ingress实现

---
## 85. moltbot/moltbot
- **GitHub**: https://github.com/moltbot/moltbot
- **语言**: Python
- **描述**: Moltin Bot
- **主要特性**: 电商聊天机器人

---
## 86. lbjlaq/Antigravity-Manager
- **GitHub**: https://github.com/lbjlaq/Antigravity-Manager
- **语言**: Python
- **描述**: 反重力管理器
- **主要特性**: Antigravity相关工具

---
## 87. anomalyco/opencode-anthropic-auth
- **GitHub**: https://github.com/anomalyco/opencode-anthropic-auth
- **语言**: Python
- **描述**: OpenCode认证
- **主要特性**: OpenCode认证集成

---
## 88. openclaw/openclaw
- **GitHub**: https://github.com/openclaw/openclaw
- **语言**: TypeScript
- **描述**: AI助手框架
- **主要特性**: 开源AI助手和代理框架

---
## 89. Draculabo/AntigravityManager
- **GitHub**: https://github.com/Draculabo/AntigravityManager
- **语言**: Python
- **描述**: Antigravity管理
- **主要特性**: Antigravity项目管理器

---
## 90. PaddlePaddle/PaddleOCR
- **GitHub**: https://github.com/PaddlePaddle/PaddleOCR
- **语言**: Python
- **描述**: 百度OCR工具
- **主要特性**: 支持多种语言的OCR识别

---
## 91. AlexanderGrooff/mermaid-ascii
- **GitHub**: https://github.com/AlexanderGrooff/mermaid-ascii
- **语言**: TypeScript
- **描述**: Mermaid图表工具
- **主要特性**: Mermaid图表ASCII渲染

---
## 92. reconurge/flowsint
- **GitHub**: https://github.com/reconurge/flowsint
- **语言**: Python
- **描述**: 流集成工具
- **主要特性**: 工作流集成平台

---
## 93. pedramamini/Maestro
- **GitHub**: https://github.com/pedramamini/Maestro
- **语言**: TypeScript
- **描述**: AI音乐生成
- **主要特性**: AI音乐创作工具

---
## 94. amantus-ai/vibetunnel
- **GitHub**: https://github.com/amantus-ai/vibetunnel
- **语言**: Python
- **描述**: Vibe隧道工具
- **主要特性**: 内网穿透工具

---
## 95. j178/prek
- **GitHub**: https://github.com/j178/prek
- **语言**: Go
- **描述**: Go密码库
- **主要特性**: Go语言加密工具库

---
## 96. steipete/Peekaboo
- **GitHub**: https://github.com/steipete/Peekaboo
- **语言**: Swift
- **描述**: macOS调试工具
- **主要特性**: macOS应用调试和分析

---
## 97. autobrr/qui
- **GitHub**: https://github.com/autobrr/qui
- **语言**: Go
- **描述**: 下载管理界面
- **主要特性**: 现代化下载管理UI

---
## 98. langchain-ai/rag-from-scratch
- **GitHub**: https://github.com/langchain-ai/rag-from-scratch
- **语言**: Python
- **描述**: RAG教程
- **主要特性**: 从零实现RAG系统

---
## 99. openai/skills
- **GitHub**: https://github.com/openai/skills
- **语言**: Python
- **描述**: OpenAI技能集
- **主要特性**: OpenAI官方技能扩展

---
## 100. disler/claude-code-hooks-mastery
- **GitHub**: https://github.com/disler/claude-code-hooks-mastery
- **语言**: Python
- **描述**: Claude代码钩子
- **主要特性**: Claude代码使用技巧

---
## 101. ankitects/anki
- **GitHub**: https://github.com/ankitects/anki
- **语言**: Rust
- **描述**: 间隔重复记忆软件
- **主要特性**: 开源记忆卡片应用

---
## 102. fish-shell/fish-shell
- **GitHub**: https://github.com/fish-shell/fish-shell
- **语言**: C++
- **描述**: 现代化终端Shell
- **主要特性**: 用户友好的命令行解释器

---
## 103. linshenkx/prompt-optimizer
- **GitHub**: https://github.com/linshenkx/prompt-optimizer
- **语言**: Python
- **描述**: 提示词优化工具
- **主要特性**: 优化LLM提示词

---
## 104. ZeroTworu/anet
- **GitHub**: https://github.com/ZeroTworu/anet
- **语言**: Python
- **描述**: AI网络工具
- **主要特性**: 神经网络相关工具

---
## 105. DataExpert-io/data-engineer-handbook
- **GitHub**: https://github.com/DataExpert-io/data-engineer-handbook
- **语言**: Python
- **描述**: 数据工程师手册
- **主要特性**: 数据工程实践指南

---
## 106. pydantic/monty
- **GitHub**: https://github.com/pydantic/monty
- **语言**: Python
- **描述**: Pydantic扩展
- **主要特性**: Python数据验证增强库

---
## 107. home-assistant/addons
- **GitHub**: https://github.com/home-assistant/addons
- **语言**: Python
- **描述**: Home Assistant插件
- **主要特性**: 家庭自动化插件集合

---
## 108. gitbutlerapp/gitbutler
- **GitHub**: https://github.com/gitbutlerapp/gitbutler
- **语言**: Rust
- **描述**: Git分支管理工具
- **主要特性**: 虚拟分支管理，增强Git功能

---
## 109. hsliuping/TradingAgents-CN
- **GitHub**: https://github.com/hsliuping/TradingAgents-CN
- **语言**: Python
- **描述**: 交易代理中文版
- **主要特性**: TradingAgents中文适配

---
## 110. public-apis/public-apis
- **GitHub**: https://github.com/public-apis/public-apis
- **语言**: Python
- **描述**: 免费API列表
- **主要特性**: 免费公共API资源汇总

---
## 111. github/gh-aw
- **GitHub**: https://github.com/github/gh-aw
- **语言**: TypeScript
- **描述**: GitHub工作流助手
- **主要特性**: GitHub Actions辅助工具

---
## 112. carlvellotti/claude-code-pm-course
- **GitHub**: https://github.com/carlvellotti/claude-code-pm-course
- **语言**: Python
- **描述**: 项目经理AI课程
- **主要特性**: AI辅助项目管理培训

---
## 113. cheahjs/free-llm-api-resources
- **GitHub**: https://github.com/cheahjs/free-llm-api-resources
- **语言**: Python
- **描述**: 免费LLM API资源
- **主要特性**: 免费大模型API列表

---
## 114. Jeffallan/claude-skills
- **GitHub**: https://github.com/Jeffallan/claude-skills
- **语言**: Python
- **描述**: Claude技能集合
- **主要特性**: 社区Claude技能

---
## 115. cinnyapp/cinny
- **GitHub**: https://github.com/cinnyapp/cinny
- **语言**: TypeScript
- **描述**: Matrix客户端
- **主要特性**: 开源Matrix聊天客户端

---
## 116. HandsOnLLM/Hands-On-Large-Language-Models
- **GitHub**: https://github.com/HandsOnLLM/Hands-On-Large-Language-Models
- **语言**: Python
- **描述**: LLM实践指南
- **主要特性**: 动手学大模型

---
## 117. vercel-labs/agent-skills
- **GitHub**: https://github.com/vercel-labs/agent-skills
- **语言**: TypeScript
- **描述**: Vercel Agent技能
- **主要特性**: Vercel平台的AI代理技能

---
## 118. google-deepmind/superhuman
- **GitHub**: https://github.com/google-deepmind/superhuman
- **语言**: Python
- **描述**: DeepMind研究工具
- **主要特性**: DeepMind研究成果工具

---
## 119. Zipstack/unstract
- **GitHub**: https://github.com/Zipstack/unstract
- **语言**: Python
- **描述**: 无代码ETL平台
- **主要特性**: 非结构化数据提取和转换

---
## 120. moonshine-ai/moonshine
- **GitHub**: https://github.com/moonshine-ai/moonshine
- **语言**: Python
- **描述**: 语音模型
- **主要特性**: 轻量级语音识别模型

---
## 121. ashishps1/awesome-system-design-resources
- **GitHub**: https://github.com/ashishps1/awesome-system-design-resources
- **语言**: Python
- **描述**: 系统设计资源
- **主要特性**: 架构设计学习资料

---
## 122. OpenCTI-Platform/opencti
- **GitHub**: https://github.com/OpenCTI-Platform/opencti
- **语言**: JavaScript
- **描述**: 威胁情报平台
- **主要特性**: 开源威胁情报管理

---
## 123. NirDiamant/RAG_Techniques
- **GitHub**: https://github.com/NirDiamant/RAG_Techniques
- **语言**: Python
- **描述**: RAG技术合集
- **主要特性**: 检索增强生成技术实践

---
## 124. huggingface/skills
- **GitHub**: https://github.com/huggingface/skills
- **语言**: Python
- **描述**: HuggingFace技能
- **主要特性**: HuggingFace生态工具

---
## 125. databricks-solutions/ai-dev-kit
- **GitHub**: https://github.com/databricks-solutions/ai-dev-kit
- **语言**: Python
- **描述**: Databricks AI开发套件
- **主要特性**: Databricks AI开发工具

---
## 126. yctimlin/mcp_excalidraw
- **GitHub**: https://github.com/yctimlin/mcp_excalidraw
- **语言**: TypeScript
- **描述**: Excalidraw MCP服务器
- **主要特性**: 白板工具的MCP集成

---
## 127. muratcankoylan/Agent-Skills-for-Context-Engineering
- **GitHub**: https://github.com/muratcankoylan/Agent-Skills-for-Context-Engineering
- **语言**: Python
- **描述**: 上下文工程技能
- **主要特性**: 上下文工程最佳实践

---
## 128. f/prompts.chat
- **GitHub**: https://github.com/f/prompts.chat
- **语言**: TypeScript
- **描述**: 提示词分享平台
- **主要特性**: 分享和发现LLM提示词

---
## 129. mihail911/modern-software-dev-assignments
- **GitHub**: https://github.com/mihail911/modern-software-dev-assignments
- **语言**: Python
- **描述**: 现代软件开发
- **主要特性**: 现代开发实践作业

---
## 130. ruvnet/ruvector
- **GitHub**: https://github.com/ruvnet/ruvector
- **语言**: Python
- **描述**: 向量数据库工具
- **主要特性**: RAG向量检索工具

---
## 131. GVCLab/PersonaLive
- **GitHub**: https://github.com/GVCLab/PersonaLive
- **语言**: JavaScript
- **描述**: 虚拟形象直播
- **主要特性**: AI虚拟主播工具

---
## 132. farion1231/cc-switch
- **GitHub**: https://github.com/farion1231/cc-switch
- **语言**: Python
- **描述**: 国家代码切换
- **主要特性**: 国际化和本地化工具

---
## 133. ruvnet/ruflo
- **GitHub**: https://github.com/ruvnet/ruflo
- **语言**: Python
- **描述**: AI工作流工具
- **主要特性**: AI工作流编排

---
## 134. NousResearch/hermes-agent
- **GitHub**: https://github.com/NousResearch/hermes-agent
- **语言**: Python
- **描述**: Hermes代理框架
- **主要特性**: NousResearch的AI代理

---
## 135. NVlabs/GR00T-WholeBodyControl
- **GitHub**: https://github.com/NVlabs/GR00T-WholeBodyControl
- **语言**: Rust
- **描述**: 机器人全身控制
- **主要特性**: NVIDIA机器人控制项目

---
## 136. microsoft/markitdown
- **GitHub**: https://github.com/microsoft/markitdown
- **语言**: TypeScript
- **描述**: 文档转换工具
- **主要特性**: Markdown转多种格式

---
## 137. K-Dense-AI/claude-scientific-skills
- **GitHub**: https://github.com/K-Dense-AI/claude-scientific-skills
- **语言**: Python
- **描述**: 科学AI技能
- **主要特性**: 科研AI工具集

---
## 138. servo/servo
- **GitHub**: https://github.com/servo/servo
- **语言**: Rust
- **描述**: 浏览器引擎
- **主要特性**: Mozilla Servo浏览器引擎

---
## 139. agentscope-ai/agentscope
- **GitHub**: https://github.com/agentscope-ai/agentscope
- **语言**: Python
- **描述**: 多代理框架
- **主要特性**: 多智能体协作开发框架

---
## 140. agentscope-ai/ReMe
- **GitHub**: https://github.com/agentscope-ai/ReMe
- **语言**: Python
- **描述**: 记忆管理
- **主要特性**: 多代理记忆共享

---
## 141. LMCache/LMCache
- **GitHub**: https://github.com/LMCache/LMCache
- **语言**: Python
- **描述**: LLM缓存工具
- **主要特性**: 大模型推理缓存库

---
## 142. xpzouying/xiaohongshu-mcp
- **GitHub**: https://github.com/xpzouying/xiaohongshu-mcp
- **语言**: Python
- **描述**: 小红书MCP服务器
- **主要特性**: 小红书API集成

---
## 143. CodebuffAI/codebuff
- **GitHub**: https://github.com/CodebuffAI/codebuff
- **语言**: Python
- **描述**: 代码优化工具
- **主要特性**: AI代码分析和优化

---
## 144. openclaw/clawhub
- **GitHub**: https://github.com/openclaw/clawhub
- **语言**: TypeScript
- **描述**: OpenClaw技能市场
- **主要特性**: OpenClaw插件和技能平台

---
## 145. msitarzewski/agency-agents
- **GitHub**: https://github.com/msitarzewski/agency-agents
- **语言**: Python
- **描述**: 代理框架
- **主要特性**: AI代理开发框架

---
## 146. FujiwaraChoki/MoneyPrinterV2
- **GitHub**: https://github.com/FujiwaraChoki/MoneyPrinterV2
- **语言**: Python
- **描述**: AI视频生成
- **主要特性**: AI自动制作视频内容

---
## 147. ItzCrazyKns/Perplexica
- **GitHub**: https://github.com/ItzCrazyKns/Perplexica
- **语言**: TypeScript
- **描述**: AI搜索引擎
- **主要特性**: 开源AI驱动的搜索引擎

---
## 148. FlowiseAI/Flowise
- **GitHub**: https://github.com/FlowiseAI/Flowise
- **语言**: TypeScript
- **描述**: 可视化LLM流程构建
- **主要特性**: 拖拽式AI流程编排

---
## 149. TheCraigHewitt/seomachine
- **GitHub**: https://github.com/TheCraigHewitt/seomachine
- **语言**: Python
- **描述**: SEO工具
- **主要特性**: AI辅助SEO分析

---
## 150. inclusionAI/AReaL
- **GitHub**: https://github.com/inclusionAI/AReaL
- **语言**: Python
- **描述**: AI现实增强
- **主要特性**: AI增强现实应用

---
## 151. microsoft/mcp-for-beginners
- **GitHub**: https://github.com/microsoft/mcp-for-beginners
- **语言**: TypeScript
- **描述**: MCP入门教程
- **主要特性**: Model Context Protocol学习

---
## 152. microsoft/hve-core
- **GitHub**: https://github.com/microsoft/hve-core
- **语言**: TypeScript
- **描述**: 微软HVE核心
- **主要特性**: 微软HVE项目核心

---
## 153. QwenLM/Qwen-Agent
- **GitHub**: https://github.com/QwenLM/Qwen-Agent
- **语言**: Python
- **描述**: 通义千问代理
- **主要特性**: 阿里Qwen Agent开发框架

---
## 154. Ed1s0nZ/CyberStrikeAI
- **GitHub**: https://github.com/Ed1s0nZ/CyberStrikeAI
- **语言**: Python
- **描述**: 网络安全AI
- **主要特性**: AI驱动的网络安全工具

---
## 155. lingfengQAQ/webnovel-writer
- **GitHub**: https://github.com/lingfengQAQ/webnovel-writer
- **语言**: Python
- **描述**: 网文写作助手
- **主要特性**: AI辅助网络小说创作

---
## 156. agentjido/jido
- **GitHub**: https://github.com/agentjido/jido
- **语言**: TypeScript
- **描述**: AI代理工具
- **主要特性**: Jido AI代理平台

---
## 157. toeverything/AFFiNE
- **GitHub**: https://github.com/toeverything/AFFiNE
- **语言**: TypeScript
- **描述**: 协作文档平台
- **主要特性**: Notion替代品，支持Markdown

---
## 158. shadcn-ui/ui
- **GitHub**: https://github.com/shadcn-ui/ui
- **语言**: TypeScript
- **描述**: React组件库
- **主要特性**: 基于Radix UI的精美组件集合

---
## 159. alibaba/page-agent
- **GitHub**: https://github.com/alibaba/page-agent
- **语言**: Python
- **描述**: 阿里页面代理
- **主要特性**: 阿里相关代理工具

---
## 160. is-a-dev/register
- **GitHub**: https://github.com/is-a-dev/register
- **语言**: JavaScript
- **描述**: 免费域名注册
- **主要特性**: 免费.is-a.dev子域名

---
## 161. teng-lin/notebooklm-py
- **GitHub**: https://github.com/teng-lin/notebooklm-py
- **语言**: Python
- **描述**: NotebookLM Python客户端
- **主要特性**: Google NotebookLM API封装

---
## 162. pbakaus/impeccable
- **GitHub**: https://github.com/pbakaus/impeccable
- **语言**: TypeScript
- **描述**: 代码质量工具
- **主要特性**: 代码审查和格式化

---
## 163. koala73/worldmonitor
- **GitHub**: https://github.com/koala73/worldmonitor
- **语言**: Python
- **描述**: 世界监控
- **主要特性**: 全球数据监控工具

---
## 164. 666ghj/BettaFish
- **GitHub**: https://github.com/666ghj/BettaFish
- **语言**: Python
- **描述**: BettaFish项目
- **主要特性**: 游戏相关项目

---
## 165. Raphire/Win11Debloat
- **GitHub**: https://github.com/Raphire/Win11Debloat
- **语言**: PowerShell
- **描述**: Windows 11精简工具
- **主要特性**: 移除Windows 11不必要组件

---
## 166. promptfoo/promptfoo
- **GitHub**: https://github.com/promptfoo/promptfoo
- **语言**: TypeScript
- **描述**: LLM评估工具
- **主要特性**: 测试和评估提示词效果

---
## 167. sepinf-inc/IPED
- **GitHub**: https://github.com/sepinf-inc/IPED
- **语言**: Java
- **描述**: 数字取证工具
- **主要特性**: 法证数字调查工具

---
## 168. ItzCrazyKns/Vane
- **GitHub**: https://github.com/ItzCrazyKns/Vane
- **语言**: TypeScript
- **描述**: 浏览器扩展
- **主要特性**: 浏览器增强工具

---
## 169. fishaudio/fish-speech
- **GitHub**: https://github.com/fishaudio/fish-speech
- **语言**: Python
- **描述**: 开源语音合成
- **主要特性**: TTS模型，支持中文

---
## 170. AstrBotDevs/AstrBot
- **GitHub**: https://github.com/AstrBotDevs/AstrBot
- **语言**: Python
- **描述**: QQ机器人框架
- **主要特性**: AstrBot QQ机器人开发框架

---
## 171. shanraisshan/claude-code-best-practice
- **GitHub**: https://github.com/shanraisshan/claude-code-best-practice
- **语言**: Python
- **描述**: Claude最佳实践
- **主要特性**: Claude代码使用技巧集

---
## 172. langflow-ai/openrag
- **GitHub**: https://github.com/langflow-ai/openrag
- **语言**: Python
- **描述**: 开源RAG平台
- **主要特性**: 可视化RAG系统构建

---
## 173. InsForge/InsForge
- **GitHub**: https://github.com/InsForge/InsForge
- **语言**: Python
- **描述**: Instagram工具
- **主要特性**: Instagram数据分析和管理

---
## 174. vectorize-io/hindsight
- **GitHub**: https://github.com/vectorize-io/hindsight
- **语言**: Python
- **描述**: 向量数据库工具
- **主要特性**: Hindsight向量引擎

---
## 175. google-ai-edge/LiteRT
- **GitHub**: https://github.com/google-ai-edge/LiteRT
- **语言**: JavaScript
- **描述**: Google边缘AI运行时
- **主要特性**: 移动端AI推理框架

---
## 176. dolthub/dolt
- **GitHub**: https://github.com/dolthub/dolt
- **语言**: Go
- **描述**: Git风格数据库
- **主要特性**: 像Git一样版本控制的SQL数据库

---
## 177. Q00/ouroboros
- **GitHub**: https://github.com/Q00/ouroboros
- **语言**: Python
- **描述**: 无限循环工具
- **主要特性**: 循环和迭代工具

---
## 178. dimensionalOS/dimos
- **GitHub**: https://github.com/dimensionalOS/dimos
- **语言**: Go
- **描述**: 操作系统
- **主要特性**: 实验性操作系统

---
## 179. SynkraAI/aiox-core
- **GitHub**: https://github.com/SynkraAI/aiox-core
- **语言**: Python
- **描述**: AI核心库
- **主要特性**: Synkra AI核心组件

---
## 180. Crosstalk-Solutions/project-nomad
- **GitHub**: https://github.com/Crosstalk-Solutions/project-nomad
- **语言**: Python
- **描述**: 项目管理工具
- **主要特性**: Project Nomad解决方案

---
## 181. sickn33/antigravity-awesome-skills
- **GitHub**: https://github.com/sickn33/antigravity-awesome-skills
- **语言**: Python
- **描述**: Antigravity技能集
- **主要特性**: Antigravity相关技能

---
## 182. langchain-ai/deepagents
- **GitHub**: https://github.com/langchain-ai/deepagents
- **语言**: Python
- **描述**: 深度代理框架
- **主要特性**: LangChain深度代理

---
## 183. YishenTu/claudian
- **GitHub**: https://github.com/YishenTu/claudian
- **语言**: Python
- **描述**: Claude集成
- **主要特性**: Claude与各种工具的集成

---
## 184. fluxerapp/fluxer
- **GitHub**: https://github.com/fluxerapp/fluxer
- **语言**: TypeScript
- **描述**: 流媒体工具
- **主要特性**: Fluxer流媒体应用

---
## 185. jarrodwatts/claude-hud
- **GitHub**: https://github.com/jarrodwatts/claude-hud
- **语言**: TypeScript
- **描述**: Claude HUD
- **主要特性**: AI助手界面

---
## 186. cloudflare/workerd
- **GitHub**: https://github.com/cloudflare/workerd
- **语言**: JavaScript
- **描述**: Cf Workers运行时
- **主要特性**: Cloudflare Workers V8引擎

---
## 187. newton-physics/newton
- **GitHub**: https://github.com/newton-physics/newton
- **语言**: Python
- **描述**: 物理引擎
- **主要特性**: 牛顿物理学模拟

---
## 188. shadps4-emu/shadPS4
- **GitHub**: https://github.com/shadps4-emu/shadPS4
- **语言**: Python
- **描述**: C++ PS4模拟器
- **主要特性**: PlayStation 4模拟器

---
## 189. langchain-ai/open-swe
- **GitHub**: https://github.com/langchain-ai/open-swe
- **语言**: Python
- **描述**: 开源软件工程
- **主要特性**: LLM软件工程工具

---
## 190. affaan-m/everything-claude-code
- **GitHub**: https://github.com/affaan-m/everything-claude-code
- **语言**: Python
- **描述**: Claude代码大全
- **主要特性**: Claude代码资源汇总

---
## 191. andrewyng/context-hub
- **GitHub**: https://github.com/andrewyng/context-hub
- **语言**: Python
- **描述**: 上下文管理
- **主要特性**: AI上下文管理工具

---
## 192. opendataloader-project/opendataloader-pdf
- **GitHub**: https://github.com/opendataloader-project/opendataloader-pdf
- **语言**: Python
- **描述**: PDF数据提取
- **主要特性**: PDF文档数据提取工具

---
## 193. mobile-dev-inc/Maestro
- **GitHub**: https://github.com/mobile-dev-inc/Maestro
- **语言**: TypeScript
- **描述**: 移动测试框架
- **主要特性**: 移动应用UI测试工具

---
## 194. louis-e/arnis
- **GitHub**: https://github.com/louis-e/arnis
- **语言**: Rust
- **描述**: 开源堡垒机
- **主要特性**: 开源堡垒机系统

---
## 195. vas3k/TaxHacker
- **GitHub**: https://github.com/vas3k/TaxHacker
- **语言**: Python
- **描述**: 税务指南
- **主要特性**: 个人税务优化指南

---
## 196. TauricResearch/TradingAgents
- **GitHub**: https://github.com/TauricResearch/TradingAgents
- **语言**: Python
- **描述**: 交易代理框架
- **主要特性**: AI量化交易框架

---
## 197. openrocket/openrocket
- **GitHub**: https://github.com/openrocket/openrocket
- **语言**: Java
- **描述**: 火箭模拟器
- **主要特性**: 开源火箭设计和仿真

---
## 198. systemd/systemd
- **GitHub**: https://github.com/systemd/systemd
- **语言**: C
- **描述**: Linux系统管理器
- **主要特性**: Linux系统和服务管理器

---
## 199. jamwithai/production-agentic-rag-course
- **GitHub**: https://github.com/jamwithai/production-agentic-rag-course
- **语言**: Python
- **描述**: 生产级RAG课程
- **主要特性**: AI RAG实战课程

---
## 200. HKUDS/LightRAG
- **GitHub**: https://github.com/HKUDS/LightRAG
- **语言**: Python
- **描述**: 轻量级RAG
- **主要特性**: 高效的检索增强生成框架

---
## 201. harry0703/MoneyPrinterTurbo
- **GitHub**: https://github.com/harry0703/MoneyPrinterTurbo
- **语言**: Python
- **描述**: AI视频生成器
- **主要特性**: AI自动制作短视频

---
## 202. tinygrad/tinygrad
- **GitHub**: https://github.com/tinygrad/tinygrad
- **语言**: Python
- **描述**: 深度学习框架
- **主要特性**: 极简深度学习框架

---
## 203. jingyaogong/minimind
- **GitHub**: https://github.com/jingyaogong/minimind
- **语言**: Python
- **描述**: 迷你大模型
- **主要特性**: 从零实现的小型LLM

---
## 204. kepano/obsidian-skills
- **GitHub**: https://github.com/kepano/obsidian-skills
- **语言**: Python
- **描述**: Obsidian技能
- **主要特性**: Obsidian笔记技巧

---
## 205. hesreallyhim/awesome-claude-code
- **GitHub**: https://github.com/hesreallyhim/awesome-claude-code
- **语言**: Python
- **描述**: Claude代码精选
- **主要特性**: Claude代码资源

---
## 206. pascalorg/editor
- **GitHub**: https://github.com/pascalorg/editor
- **语言**: TypeScript
- **描述**: 在线编辑器
- **主要特性**: Pascal编辑器

---
## 207. mvanhorn/last30days-skill
- **GitHub**: https://github.com/mvanhorn/last30days-skill
- **语言**: Python
- **描述**: 30天技能
- **主要特性**: 学习30天技能项目

---
## 208. BerriAI/litellm
- **GitHub**: https://github.com/BerriAI/litellm
- **语言**: Python
- **描述**: 统一LLM接口
- **主要特性**: 调用100+大模型的统一API

---
## 209. usestrix/strix
- **GitHub**: https://github.com/usestrix/strix
- **语言**: TypeScript
- **描述**: AI界面组件
- **主要特性**: AI UI组件库

---
## 210. letta-ai/claude-subconscious
- **GitHub**: https://github.com/letta-ai/claude-subconscious
- **语言**: Python
- **描述**: Claude潜意识
- **主要特性**: Claude记忆和潜意识

---
## 211. Yeachan-Heo/oh-my-claudecode
- **GitHub**: https://github.com/Yeachan-Heo/oh-my-claudecode
- **语言**: Python
- **描述**: Claude代码助手
- **主要特性**: Oh My Claude Code扩展

---
## 212. Vaibhavs10/insanely-fast-whisper
- **GitHub**: https://github.com/Vaibhavs10/insanely-fast-whisper
- **语言**: Python
- **描述**: 极速Whisper
- **主要特性**: 优化版Whisper推理

---
## 213. datalab-to/chandra
- **GitHub**: https://github.com/datalab-to/chandra
- **语言**: Python
- **描述**: 数据实验室
- **主要特性**: 数据分析和可视化

---
## 214. figma/mcp-server-guide
- **GitHub**: https://github.com/figma/mcp-server-guide
- **语言**: TypeScript
- **描述**: Figma MCP指南
- **主要特性**: Figma MCP集成文档

---
## 215. SakanaAI/AI-Scientist-v2
- **GitHub**: https://github.com/SakanaAI/AI-Scientist-v2
- **语言**: Python
- **描述**: AI科学家
- **主要特性**: 自动化AI研究工具

---
## 216. FreeCAD/FreeCAD
- **GitHub**: https://github.com/FreeCAD/FreeCAD
- **语言**: C++
- **描述**: 开源CAD软件
- **主要特性**: 参数化3D建模工具

---
## 217. onyx-dot-app/onyx
- **GitHub**: https://github.com/onyx-dot-app/onyx
- **语言**: TypeScript
- **描述**: AI笔记应用
- **主要特性**: 开源AI笔记和知识库

---
## 218. manaflow-ai/cmux
- **GitHub**: https://github.com/manaflow-ai/cmux
- **语言**: Python
- **描述**: 多路复用
- **主要特性**: AI任务多路复用工具

---
## 219. luongnv89/claude-howto
- **GitHub**: https://github.com/luongnv89/claude-howto
- **语言**: Python
- **描述**: Claude使用指南
- **主要特性**: Claude AI使用教程

---
## 220. fastfetch-cli/fastfetch
- **GitHub**: https://github.com/fastfetch-cli/fastfetch
- **语言**: C
- **描述**: 系统信息工具
- **主要特性**: 比Neofetch更快的系统信息

---
## 221. freeCodeCamp/freeCodeCamp
- **GitHub**: https://github.com/freeCodeCamp/freeCodeCamp
- **语言**: JavaScript
- **描述**: 编程学习平台
- **主要特性**: 免费编程学习和认证

---
## 222. sherlock-project/sherlock
- **GitHub**: https://github.com/sherlock-project/sherlock
- **语言**: Python
- **描述**: 用户名搜索工具
- **主要特性**: 在社交网络搜索用户名

---
## 223. Dimillian/Skills
- **GitHub**: https://github.com/Dimillian/Skills
- **语言**: Swift
- **描述**: Apple开发技能
- **主要特性**: Apple平台开发技能

---
## 224. neovim/neovim
- **GitHub**: https://github.com/neovim/neovim
- **语言**: Vim Script
- **描述**: 现代化Vim
- **主要特性**: 可扩展的文本编辑器

---
## 225. jwasham/coding-interview-university
- **GitHub**: https://github.com/jwasham/coding-interview-university
- **语言**: Python
- **描述**: 编程面试指南
- **主要特性**: 成为软件工程师的学习路径

---
## 226. axios/axios
- **GitHub**: https://github.com/axios/axios
- **语言**: JavaScript
- **描述**: HTTP客户端
- **主要特性**: Promise-based HTTP客户端

---
## 227. aaif-goose/goose
- **GitHub**: https://github.com/aaif-goose/goose
- **语言**: Python
- **描述**: AI代理框架
- **主要特性**: AAI开源AI代理工具

---
## 228. coleam00/Archon
- **GitHub**: https://github.com/coleam00/Archon
- **语言**: TypeScript
- **描述**: 终端AI界面
- **主要特性**: 终端AI聊天界面

---
## 229. forrestchang/andrej-karpathy-skills
- **GitHub**: https://github.com/forrestchang/andrej-karpathy-skills
- **语言**: Python
- **描述**: Karpathy技能
- **主要特性**: Andrej Karpathy分享的技能

---
## 230. goharbor/harbor
- **GitHub**: https://github.com/goharbor/harbor
- **语言**: Go
- **描述**: 云原生镜像仓库
- **主要特性**: 企业级容器镜像仓库

---
## 231. jqlang/jq
- **GitHub**: https://github.com/jqlang/jq
- **语言**: C
- **描述**: 命令行JSON工具
- **主要特性**: 处理JSON的命令行工具

---
## 232. mattpocock/skills
- **GitHub**: https://github.com/mattpocock/skills
- **语言**: TypeScript
- **描述**: 技能集合
- **主要特性**: M端Pocock的技能集

---
## 233. multica-ai/multica
- **GitHub**: https://github.com/multica-ai/multica
- **语言**: Python
- **描述**: 多模态AI框架
- **主要特性**: 多模态模型集成

---
## 234. shiyu-coder/Kronos
- **GitHub**: https://github.com/shiyu-coder/Kronos
- **语言**: Python
- **描述**: 时间序列工具
- **主要特性**: 时间序列分析和预测

---
